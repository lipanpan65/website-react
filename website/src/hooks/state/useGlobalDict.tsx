...
const reducer = (state: StateType, action: ActionType): StateType => {
  switch (action.type) {
    ...
    case 'UPDATE_PARAMS':
      console.log("Reducer received UPDATE_PARAMS:", action.payload.params);
      return {
        ...state,
        params: {
          ...state.params, // 合并旧的 params
          ...action.payload.params, // 合并新的 params
        },
      };
    default:
      return state;
...
export const GlobalProvider = ({ children }: { children: ReactNode }) => {
  ...
  const enhancedDispatch: EnhancedDispatch = (action) => {
    if (typeof action === 'function') {
      action(dispatch); // 如果是函数，执行并传递 dispatch
    } else {
      dispatch(action); // 如果是 ActionType，直接派发
    }
  };
