...
// https://juejin.cn/post/7273025938937528360#heading-8

export class MarkDownTOC extends Component<any, any>{
	// declarative:any = false
	static propTypes = {
	...
	scrollEventLock: boolean | undefined;
	scrollTimeout: any;
	...
	constructor(props: any) {
		...
		this.state = {
			currentListNo: '',
			navStructure: [],
			addTargetTimeout: null,
			scrollEventLock: null,

		};
	}

	safeScrollTo(element: any, top: any, left: any = 0, smooth?: any) {
		if (!element) return
	...
	refreshNav(source: any) {
		if (this.addTargetTimeout) {
			clearTimeout(this.addTargetTimeout);
		}
		console.log("this.getNavStructure(source)===>", this.getNavStructure(source))
	...
	componentDidMount() {
		...
		// console.log("componentDidMount.source===>", source)
	...
	getNavStructure(source?: any) {
		...
		console.log("source===>", contentWithoutCode)
		...
		// const pattOfTitle = /#\s([^#\n]+)\n*/g;
		// const pattOfTitle = /#\s([^#\n]+)\n*/g;
		console.log("pattOfTitle===>", pattOfTitle)
		...
		const navData = matchResult.map((r: any, i: any) => ({
			index: i,
		...
		for (let i = 0; i < navData.length; i++) {
			...
			const { arr }: any = matchStack[matchStack.length - 1];
	...
	winScroll = throttle(() => {
		...
		const distanceList = newHeadingList.map((h: any) => h.distanceToTop);
		...
		if (currentNavElement) {
			...
			const { scrollTop: containerScrollTop, offsetHeight: containerOffsetHeight }: any = container
	...
	render() {
		const tBlocks = this.getNavStructure(this.props.source).map((t: any) => {
			console.log("getNavStructure===>", t)
